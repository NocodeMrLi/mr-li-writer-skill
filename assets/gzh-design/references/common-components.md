...
Markdown 表格必须转换为真实 `<table>`，不能把 `| 字段 |` 和 `| --- |` 当普通段落输出，也不能用卡片假装表格。手机端优先让表头和单元格自然换行：1～4 列占满内容区且不启用横向滚动；5 列及以上才保留有上限的局部横向滚动。四列及以上使用 `12px` 字号和 `5px` 横向内边距。
...
<section style="width:100%;max-width:100%;box-sizing:border-box;margin:0 0 24px;">
  <table style="width:100%;max-width:100%;table-layout:fixed;border-collapse:collapse;border-spacing:0;box-sizing:border-box;">
    <thead><tr>
      <th style="box-sizing:border-box;padding:8px 6px;border:1px solid #FEE2E2;background:#FEF2F2;color:#991B1B;font-size:13px;font-weight:800;text-align:left;line-height:1.55;word-break:normal;overflow-wrap:anywhere;white-space:normal;"><span leaf="">{{列标题}}</span></th>
      <th style="box-sizing:border-box;padding:8px 6px;border:1px solid #FEE2E2;background:#FEF2F2;color:#991B1B;font-size:13px;font-weight:800;text-align:left;line-height:1.55;word-break:normal;overflow-wrap:anywhere;white-space:normal;"><span leaf="">{{列标题}}</span></th>
    </tr></thead>
    <tbody><tr>
      <td style="box-sizing:border-box;padding:8px 6px;border:1px solid #FEE2E2;background:#FFFFFF;color:#374151;font-size:13px;line-height:1.55;word-break:normal;overflow-wrap:anywhere;white-space:normal;"><span leaf="">{{内容}}</span></td>
      <td style="box-sizing:border-box;padding:8px 6px;border:1px solid #FEE2E2;background:#FFFFFF;color:#374151;font-size:13px;line-height:1.55;word-break:normal;overflow-wrap:anywhere;white-space:normal;"><span leaf="">{{内容}}</span></td>
    </tr></tbody>
  </table>
</section>
...
按主题替换边框、表头底色、表头文字色和斑马纹底色，不改变 `width/max-width/table-layout/box-sizing/word-break/overflow-wrap/white-space` 这些自适应约束。1～4 列使用 `width:100%;max-width:100%` 且不设置 `min-width`；5 列及以上才增加滚动容器，最小宽度按 `max(420px, 列数 × 92px)` 计算并以 `680px` 为上限。任何列数都不得在 `<th>/<td>` 使用 `word-break:keep-all`。
