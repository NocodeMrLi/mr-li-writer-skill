...
- **跨端稳定排版**：公众号目录、标题、金句和表格按 PC/手机预览自适应，避免长标题末行只剩一两个字
...
公众号正文采用内联样式、固定表格布局、语义分行和容器内换行等兼容约束，并经过桌面与手机视口检查。前端标签会优先使用读者可理解的内容口径，避免把后台生产词、编辑立场词暴露到阅读端。微信公众号编辑器仍可能随版本调整过滤规则，重要文章发布前应在公众号后台分别完成手机端和 PC 端预览。
