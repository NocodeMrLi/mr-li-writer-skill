...
- **来源控制**：用户给的链接/文件只作为种子资料，硬信息会继续核对与该事实领域匹配的官方、权威和最新来源；利益相关培训或服务机构只作内部线索，不在正文显名引流
...
用户提供链接、文件或截图时，Skill 会把它们当作种子资料，并继续补搜权威、最新和匹配资料。即使没有用户种子资料，只要涉及政策、考试、报名、资格、价格等硬信息，也必须核验来源。正式写作、排版或交付前，任务状态需要记录 `original_prompt`、`research_scope`，并在 `source_entities` 中逐一登记来源角色、事实适用范围和读者端可见性，再运行资料范围校验：
...
商业利益相关来源的显名拦截词维护在 `references/commercial-source-terms.txt`，但词表只是一层回归防线，不是主要判断机制。系统还会检查培训、网校、辅导、题库、课程、咨询等通用商业特征，并通过 `source_entities` 对任意新机构做角色审计。权威来源必须与具体事实领域匹配；第三方信息无法由官方确认时，只能匿名、降低结论强度并明确“尚待官方确认”，不能把培训机构汇总写成确定事实。
