...
公众号正文采用内联样式、语义分行和跨端表格等兼容约束，并经过桌面与手机视口检查。表格在桌面宽度内完整展示，在窄屏使用有限宽度的局部横向滚动，避免把短日期和短词压成逐字竖排。短金句和公式会按语义分行，长解释段落会保持自然左对齐。所有主题共用严格的读者标签白名单，未登记的内部能力名、后台生产词或编辑立场词会自动回退；标签仍需按文章内容选择，不能机械固定。微信公众号编辑器仍可能随版本调整过滤规则，重要文章发布前应在公众号后台分别完成手机端和 PC 端预览。
