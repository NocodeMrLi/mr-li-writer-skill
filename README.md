...
- **来源控制**：用户给的链接/文件只作为种子资料，硬信息会继续核对官方、权威和最新来源，避免把利益相关培训或服务机构写成正文背书
...
用户提供链接、文件或截图时，Skill 会把它们当作种子资料，并继续补搜权威、最新和匹配资料。正式写作、排版或交付前，任务状态需要记录 `original_prompt` 和 `research_scope`，再运行资料范围校验：
...
公众号正文采用内联样式、语义分行和跨端表格等兼容约束，并经过桌面与手机视口检查。表格在桌面宽度内完整展示，在窄屏使用有限宽度的局部横向滚动，避免把短日期和短词压成逐字竖排。短金句和公式会按语义分行，长解释段落会保持自然左对齐。前端标签会优先使用读者可理解的内容口径，避免把后台生产词、编辑立场词暴露到阅读端。微信公众号编辑器仍可能随版本调整过滤规则，重要文章发布前应在公众号后台分别完成手机端和 PC 端预览。
