...
局部授权不会被扩大解释：例如“标题可以自动匹配”“标签你来定”只代表对应部分可自动处理，不代表发布平台、内容目标、创作方向和平台交付样式都已确认。

Skill 还内置了长期记忆防火墙：长期偏好、宿主 memory、standing instruction、上次任务默认值或其他 skill 的使用习惯，不能替代本次 `mr-li-writer` 的确认。只有当前任务中用户明确授权“自动匹配 / 不用问 / 直接处理”，才会进入自动匹配。
