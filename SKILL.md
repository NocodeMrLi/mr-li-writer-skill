---
name: mr-li-writer
description: "面向中文内容创作的编辑判断、研究、策划、写作与平台原生交付工作流。根据标题、素材、参考资料、核心想法或模糊灵感，先识别读者任务、材料条件和创新必要性，再按需完成语境扫描、可写性判断与方向校准，并区分发布平台与内容目标，进行事实检索、观点设计、平台原生正文写作、标题策划和对应交付。适用于公众号文章、知乎回答、小红书原生笔记、官网/网页、个人博客、行业解读、实用指南和基于素材的扩展研究写作。"
---
...
19. **交付物必须真实可打开**：先生成并核验实际文件，再通过宿主的原生附件或文件卡片交付。文件任务只有在任务页出现可点击打开的文件卡片后才算交付完成；不得把本地绝对路径、代码样式文件名或文件清单冒充交付。宿主无附件能力或附件调用失败时必须如实说明阻断，且不得把任务标记为已交付。
...
如果脚本阻断，必须把脚本输出的问题卡原样交给用户确认，不得自行缩写、合并平台与主题、裁剪选项或改成宿主选择器里不完整的 2-3 个选项。问题卡采用依赖安全的两阶段流程：发布平台未知时，第一轮只问发布平台，并可同时问不依赖平台的内容目标和创作方向；不得在同一轮或同一个宿主选择器里生成任何平台专属交付样式。用户选定平台后，再运行入口检查或根据已确认状态发出第二轮短问题，只展示该平台的完整交付样式选项。两轮都保留“补充说明/自行输入”。用户回复后，把真实确认写入任务状态 JSON，并在后续正式动作中传入 `--task-state`：
...
按 `references/platform-native-protocol.md` 的“最低必要询问”执行。只问会改变文章形态或交付结果的问题，已经明确的信息不重复问；同一阶段缺少多个关键项时合并询问，通常不超过 3 个短问题。平台交付样式依赖发布平台，平台未知时必须延后到第二阶段，不得为追求“一次问完”提前展示公众号或其他平台样式。
...
如果同一任务同时缺少多个信息，优先使用 `validate_task_intake.py --emit-question-card` 输出的问题卡。平台已知时可合并剩余缺口；平台未知时严格分两阶段，第一阶段确认平台/内容目标/创作方向，第二阶段只确认该平台交付样式。可在方向候选里吸收目标读者、立场和篇幅差异，减少单独提问。问题卡必须覆盖当前任务真正影响结果的项，但不得为了显得完整无止境追问。
...
- 发布平台未确认时，不得创建平台交付样式问题、选项或原生选择器；选定平台后必须重新生成样式问题，且只能包含该平台选项。用户在第一轮选择“知乎”后，第二轮绝不能出现摸鱼绿、橄榄手记等公众号排版主题。
...
| 未知平台 | 先确认发布平台；本阶段不得展示任何平台专属交付样式 |
...
- 最终交付必须调用宿主提供的原生附件、文件卡片或 artifact 能力，把实际文件附到消息中；预览 HTML 排在第一位，源稿随后。
- 禁止输出 `[用途名](C:\\...\file.html)`、`[用途名](/绝对路径/file.html)` 或其他本地 Markdown 链接并宣称可点击。它们在部分宿主中只会显示为打不开的蓝字。
- 仅列出工作区路径不算交付。宿主没有原生附件能力或附件调用失败时，可以给出纯文本路径帮助找回文件，但必须明确本次“文件已生成、可点击交付未完成”，不得把任务标记为已交付。
...
所有宿主和所有智能体都必须按同一顺序完成文件交付：

1. 运行 `scripts/validate_delivery_bundle.py`，读取其输出的 `ATTACHMENT_REQUIRED` 清单。
2. 调用当前宿主真实的原生附件、文件卡片或 artifact 工具；不能只在回复里打印文件名、表格、代码样式路径或 Markdown 本地链接。
3. 有预览 HTML 时，预览 HTML 必须作为第一张文件卡片；随后依次附加干净 HTML、平台原生正文和标题策略。无预览时先附平台原生正文。
4. 检查附件工具返回结果；宿主支持回读时，还要确认任务页已经出现对应文件卡片。只有这一步成功后，才可以使用“已交付”“可点击打开”“完成”等表述并结束任务。
5. 附件失败时，先用短 ASCII 文件名、当前 workspace 内文件和单文件预览重试一次。仍失败则报告“文件已生成，但可点击交付被宿主阻断”，仅列出工作区路径不算交付，也不得把任务标记为已交付。

这个门禁适用于 WorkBuddy、Codex、Claude Code、Cursor 和其他 Agent。文件卡片、可点击蓝色附件名称或宿主 artifact 卡片的视觉样式可以不同，但点击后必须能打开或预览真实文件。
...
4. 使用宿主原生附件/文件卡片附上真实文件；不要用本地 Markdown 路径链接代替附件。交付前确认预览 HTML 是第一附件且文件非空，并检查 attachment tool result 是否成功。
5. 若当前宿主明确不支持原生附件或附件调用失败，纯文本绝对路径只用于找回文件，不构成最终交付；说明能力边界并保持任务为“可点击交付未完成”。
