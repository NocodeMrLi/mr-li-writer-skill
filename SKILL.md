...
16. **先确认形态，再开始制作**：发布平台决定排版与交付样式，未经用户确认（或用户明确授权自动选择）不得进入排版环节；宁可多问一句，不拿默认值赌用户预期。
...
2. 适合预览的任务（报告、方案、指南等）应生成独立 HTML 交付物，含完整图文；图片使用 `<figure><img><figcaption>` 结构，相对路径引用 `assets/` 下文件。
