...
- 公众号：固定排版，只确认公众号排版主题由用户选择还是系统推荐。
- 知乎：语境不明确时确认写回答还是专栏，并确认是否需要 HTML 排版预览。
- 小红书：确认是否需要 HTML 排版预览。
- 官网/网页：确认内容目标和发布环境/CMS 约束；HTML 仍需同时保留标题策略和网页原文。
- 个人博客：确认使用 Markdown / CMS 富文本 / 静态 HTML 中哪种发布格式。
- 未知平台：确认平台实际接收的内容格式。
...
- 市面常见写法和重复风险
- 值得保留的用户原始想法
- 更推荐的 1-3 个写作方向，或明确说明“原方向已经合适，无需换立意”
...
10. 标题/金句换行必须按读者端真实宽度设计。封面标题、目录卡标题、引用金句、居中大字和公式句，不能只依赖浏览器自动断行；长度超过一行时先按语义拆分、缩短或换成两段式表达，避免最后一行只剩 1-3 个字、一个标点或半个词。只有短金句、短公式、短判断适合居中；一旦引用内容已经接近段落，或包含多句解释、人物说法、连续判断，必须使用左对齐自然换行，不得把一整段文字居中排成大字块。
